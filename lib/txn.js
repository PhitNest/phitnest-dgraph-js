...
class Txn {
    ...
    mutateGraphQL(mutation) {
        return this.mutate({
            setJson: util_1.toPredicateMap(mutation.obj),
    ...
    async queryGraphQL(query) {
        return Object.fromEntries(Object.entries((await this.query(query)).data).map(([key, value]) => [
            key,
            value.map(predicateMap => util_1.fromPredicateMap(predicateMap)),
        ]));
    }
    query(q, options) {
    ...
    async mutate(mu) {
        ...
        try {
            ...
            throw util_1.isAbortedError(e) || util_1.isConflictError(e) ? errors_1.ERR_ABORTED : e;
        }
    }
    async commit() {
        if (this.finished) {
        ...
        try {
            ...
            throw util_1.isAbortedError(e) ? errors_1.ERR_ABORTED : e;
        }
    }
    async discard() {
        if (this.finished) {
